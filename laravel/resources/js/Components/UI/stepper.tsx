...
function useStepper() {
    ...
    if (context === undefined) throw new Error('useStepper must be used within a StepperProvider');
...
const StepIcon = React.forwardRef<HTMLDivElement, StepIconProps>((props, ref) => {
    ...
    return React.useMemo(() => {
        ...
        if (isCurrentStep) {
            ...
            if (isLoading) {
                return <Loader2 className={ny(iconVariants({ size }), 'animate-spin')} />;
            }
        }
        if (Icon) {
            return (
...
function StepLabel({ isCurrentStep, opacity, label, description }: StepLabelProps) {
